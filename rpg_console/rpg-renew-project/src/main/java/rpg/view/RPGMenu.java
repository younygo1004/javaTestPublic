...
public class RPGMenu {
    ...
    public void viewUserInventory() {
        ...
        List<Item> itemList = manager.getUserBag().getItemList();
    ...
    public void talkAboutWeather(NPCDTO selectedNPC) {
        System.out.println(manager.getUserInfo().getName() + ": 오늘 날씨 어때?");
    ...
    public void talkAboutTrip(NPCDTO selectedNPC) {
        System.out.println(manager.getUserInfo().getName() + ": 우리 놀러갈래?!");
        if (selectedNPC.getLike() > 50) {
            ...
            System.out.println(manager.getUserInfo().getName() + " 은/는 버려졌다....");
        }
    }

    public void talkAboutFood(NPCDTO selectedNPC) {
        System.out.println(manager.getUserInfo().getName() + ": 어제 뭐 먹고 잤니..?");
    ...
    public void presentGift(NPCDTO selectedNPC) {
        ...
        sc.nextLine();
        Gift givenGift = userInfo.getBag().getGiftList().get(selectedGift);
    ...
    public void showGiftList() {
        List<Gift> giftList = manager.getUserBag().getGiftList();
    ...
    public void goToItemShop() {
        ...
        Item[] itemList = manager.getItemShopItemList(shopType);
        showItemList(itemList);
        System.out.print("어느 물건을 달라 할까? : ");
        int itemIndex = sc.nextInt() - 1;

        Item buyItem = switch (shopType) {
            case 1 -> manager.getBuyClothes(itemIndex);
            case 2 -> manager.getBuyGift(itemIndex);
            default -> throw new InputMismatchException();
        };

        UserDTO userInfo = manager.getUserInfo();
        if (userInfo.getMoney() < buyItem.getPrice()) {
            System.out.println("돈이 없다. 아르바이트를 하러 가볼까?");
            return;
        }

        manager.buyItem(buyItem);
        ...
        userInfo.minusMoney(buyItem.getPrice());
    ...
    public void showItemList(Item[] itemList) {
